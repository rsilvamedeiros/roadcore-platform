...
export function Footer({ tenant }: FooterProps) {
  ...
  return <footer className="bg-[#08152e] text-white">
    <div className="page-shell grid gap-10 py-14 md:grid-cols-[1.6fr_1fr_1fr_1fr]">
      <div><div className="flex items-center gap-3 font-semibold"><BrandMark />{theme.name}</div><p className="mt-4 max-w-sm text-sm leading-6 text-slate-400">Conectamos pessoas, veículos e operações para fazer negócios avançarem com segurança.</p></div>
      ...
      <div><h3 className="text-sm font-semibold">Serviços</h3><div className="mt-4 flex flex-col gap-3 text-sm text-slate-400"><Link href={`/${tenant}/contact`}>Venda seu veículo</Link><Link href={`/${tenant}/contact`}>Financiamento</Link><Link href={`/${tenant}/contact`}>Fretes</Link></div></div>
